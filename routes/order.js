...
router.route('/time-slot').post(isAuthenticatedUser, authorizeRoles('admin'), addAvailablePickupTimeSlots);
...
router.route('/admin/orderHistory-active/:restaurantId').get(isAuthenticatedUser, getActiveOrdersByBranch);

// Get Non-Active Orders by Restaurant ID: GET /api/admin/orderHistory-nonActive/:restaurantId (Admin role required)
router.route('/admin/orderHistory-nonActive/:restaurantId').get(isAuthenticatedUser, getNonActiveOrdersByBranch);
